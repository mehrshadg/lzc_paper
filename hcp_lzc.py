...
from neuro_helper.measurement import calc_lzc
...
def do_a_subject(tpt: TemplateMap, input_):
    data, fs, label = load_raw_files(input_, tpt.space, concat_scans)
    print(f"Calculating LZC on {label}")
    data, freq_l, freq_h = fir_filter(data, fs, max_freq_low=0.01, pass_type="hp")
    data = tpt.parcellate(data)
    n_sample = data.shape[1]
    return calc_lzc(data, n_sample / np.log2(n_sample), 5)
...
def run_script(tpt: TemplateMap):
    for task in task_order(tpt.space, with_rest=True):
        ...
        for key, file_infos in files_dict.items():
            output_file = out_of(f"fmri-hcp-{task}.lzc.rois-{tpt.name}.scan-{key}-{tpt.space}.npy", False)
...
def find_files(**kwargs):
    ...
    tpt = kwargs["template"]
    files = glob.glob(
        out_of(f"fmri-hcp-{task}.lzc.rois-{tpt.name}.scan-{ANYTHING}-{tpt.space.dropped_cortex}.npy", False))
    files.sort()
    return files
